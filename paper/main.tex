...
\subsection{Separation Dependence of Merger Timescales}\label{sec:results-timevsep}
    ...
    % For the same physical separation, the merger timescale is lower than for low-mass 
    % This does that the overall merger timescales are lower for high-mass systems. 
    ...
    \begin{figure}[htb]
        ...
        \caption{The median time until merger as a function of redshift for low-mass (green) and high-mass (pink) pairs that will merge by $z=0$. Shaded regions represent the 1st and 3rd quartile spread on the median. %Pairs at each redshift are selected if they have experienced first infall and are pre-merger. 
        % 
...
\subsection{Redshift Dependence of Merger Timescales}\label{sec:results-timevredshift}
    ...
    \label{sec:results-phys}
        ...
        In the rightmost top panel, the median merger times converge for $z=0-0.5$ and $z>1.5$, as expected based on results from the full sample shown in Fig. \ref{fig:timescales}. %almost mimicking our findings from the full sample analysis. 
...
\section{Discussion} \label{sec:discussion}
    ...
    However, 
    % In~\cite{RG2015}, they quantify the galaxy major merger rate as a function of the stellar mass of the descendent subhalo. 
    % It is unclear, however, if the major merger rate for galaxies with stellar mass $\ms<10^{10}\Msun$ is actually discrepant between simulations and observations, or if the observational sam
    ...
    Bring up assumption of isolation criteria and it's drawbacks and strengths. Reiterate that our findings are for isolated pairs. -> Focus of future work to explore the impact of pair selection criteria on pairs in a variety of different galaxy environments. 
            ...
            % As discussed in Chamberlain, this is doable, the abundance matching prescription was formulated with the stellar mass errors already taken into account, so this isn't introducing significantly more error in the analysis
            % quote the stellar mass errors being folded in at high z 
        \end{itemize}
